...
opensave.loadFile = function (theButtonID, theFilename, theData64){
	...
	if (opensave.buttons[theButtonID].handler){
		...
		if(opensave.buttons[theButtonID].onComplete){
			opensave.buttons[theButtonID].onComplete(but);
		}
...
opensave.loadFileCancelled = function(id){
	var but = opensave.buttons[id];
	if(but.onCancel){
		but.onCancel("cancelled", but);
	}
}
opensave.loadFileError = function(id){
	var but = opensave.buttons[id];
	if(but.onError){
		but.onError("error", but);
	}
}
...
opensave.saveResults = function(status, id, url){
	var but = opensave.buttons[id];
	if(status == "error"){
		if(but.onError){
			but.onError(but);
...
opensave.progress = function (id, loaded, total){
	var but = opensave.buttons[id];
	if(but.onProgress){
		but.onProgress(but, loaded, total);
	}
}
...
opensave.make = function (options){
	...
	confObj.onComplete	= (options.onComplete) ? options.onComplete : null;
	confObj.onError		= (options.onError) ? options.onError : null;
	confObj.onCancel	= (options.onCancel) ? options.onCancel : null;
	confObj.onProgress	= (options.onProgress) ? options.onProgress : null;
